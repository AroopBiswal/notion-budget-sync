...
LAUNCHER_BUILD="$ROOT/assets/icons/build/launcher"
...
# --- Compile the TCC-primer launcher (cached, universal) ---------------
# launcher is the CFBundleExecutable — a native Mach-O binary so macOS
# shows the app's real name in TCC dialogs instead of "bash 2.1".
if [ "$LAUNCHER_MODE" = "swift" ] && [ -f "$LAUNCHER_SRC" ]; then
    mkdir -p "$(dirname "$LAUNCHER_BUILD")"
    NEEDS_REBUILD=0
    if [ ! -x "$LAUNCHER_BUILD" ] || [ "$LAUNCHER_SRC" -nt "$LAUNCHER_BUILD" ]; then
        NEEDS_REBUILD=1
    fi
    if [ "$NEEDS_REBUILD" = "1" ]; then
        echo "Compiling TCC launcher: $LAUNCHER_BUILD"
        IFS=',' read -r -a ARCH_LIST <<<"${APP_IT_SWIFT_ARCHS:-arm64,x86_64}"
        ARCH_BINS=()
        for arch in "${ARCH_LIST[@]}"; do
            arch_clean="$(echo "$arch" | tr -d ' ')"
            BIN="$LAUNCHER_BUILD.$arch_clean"
            if swiftc -O "$LAUNCHER_SRC" \
                -o "$BIN" \
        ...
        if [ "${#ARCH_BINS[@]}" -eq 0 ]; then
            swiftc -O "$LAUNCHER_SRC" -o "$LAUNCHER_BUILD" -framework Foundation
        elif [ "${#ARCH_BINS[@]}" -eq 1 ]; then
            mv "${ARCH_BINS[0]}" "$LAUNCHER_BUILD"
        else
            lipo -create "${ARCH_BINS[@]}" -output "$LAUNCHER_BUILD"
            rm -f "${ARCH_BINS[@]}"
        fi
    fi
fi
...
for entry in "${APPS[@]}"; do
    ...
    if [ "$LAUNCHER_MODE" = "swift" ]; then
        ...
        # Compile and install the per-app launcher (substitutes PROJECT_ROOT and APP_NAME)
        if [ -f "$LAUNCHER_SRC" ]; then
            TMP_LAUNCHER_SRC="$(mktemp /tmp/launcher_XXXXXX.swift)"
            substitute "$LAUNCHER_SRC" \
                "__PROJECT_ROOT__=$ROOT" \
                "__APP_NAME__=$APP_NAME" \
                > "$TMP_LAUNCHER_SRC"
            if swiftc -O "$TMP_LAUNCHER_SRC" -o "$MACOS/launcher" -framework Foundation 2>/dev/null; then
                chmod +x "$MACOS/launcher"
            else
                echo "WARN: launcher compile failed — falling back to run script as CFBundleExecutable" >&2
            fi
            rm -f "$TMP_LAUNCHER_SRC"
        fi
    fi
