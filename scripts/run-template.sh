...
if [ -z "$CHOSEN_PORT" ]; then
    ...
    # TCC probe: macOS Sequoia+ blocks ~/Documents, ~/Desktop, ~/Downloads for
    # apps without explicit user consent. /bin/ls bypasses TCC (system utility);
    # python3 is subject to the full check. Running python3 from the FOREGROUND
    # run script (before any background spawn) triggers the system consent dialog
    # for com.user.notion-budget-sync so the user can Allow before we background.
    if ! python3 -c "import os; os.listdir('.')" > /dev/null 2>&1; then
        /usr/bin/open "x-apple.systempreferences:com.apple.preference.security?Privacy_FilesAndFolders"
        /usr/bin/osascript -e "display alert \"$APP_NAME: Folder Access Required\" message \"macOS blocked access to the project folder:\n$PROJECT_ROOT\n\nIn the Privacy & Security window that just opened:\n  1. Scroll to find '$APP_NAME'\n  2. Turn the toggle ON\n\nThen click the app icon again.\""
        rm -f "$PID_FILE" "$PORT_FILE"
        exit 1
    fi

    # Detach the dev server from the wrapper's process group so SIGHUP can't
    # kill it when the wrapper window closes. macOS ships setsid only on some
    # versions; nohup+disown is the universal fallback.
    ...
    if [ "$READY" != "1" ]; then
        ...
        rm -f "$PID_FILE" "$PORT_FILE"
        # Detect TCC EPERM: python3 opened but couldn't read the script file
        if echo "$TAIL" | grep -q "Operation not permitted"; then
            /usr/bin/open "x-apple.systempreferences:com.apple.preference.security?Privacy_FilesAndFolders"
            /usr/bin/osascript -e "display alert \"$APP_NAME: Folder Access Required\" message \"macOS blocked the server from reading project files.\n\nIn the Privacy & Security window that just opened:\n  1. Scroll to find '$APP_NAME'\n  2. Turn the toggle ON\n\nThen click the app icon again.\""
            exit 1
        fi
